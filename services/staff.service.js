...
const updateStaffService = async (id, data) => {
  try {
    ...
    if (identificationNumber) {
      ...
      if (staffExist && identificationNumber !== staff.identificationNumber)
    ...
    if (phoneNumber) {
      ...
      if (staffExist && phoneNumber !== staff.phoneNumber)
        return handleError('Staff already exist', 400)
    }
...
const verifyRequestStaff = async (data) => {
  try {
    ...
    if (skills) {
      ...
      if (techStacks.length !== techStacksId.length)
        return handleError('Tech stacks incorrect', 400)
    }
  } catch (error) {
    logger.error(error.message)
    console.log(error)
